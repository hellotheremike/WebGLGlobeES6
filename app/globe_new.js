var GlobeNew = function (container, opts) {
	var config = {
		percent: 0,
		lat: 0,
		lng: 0,
	...
	function regenerateGlobe() {
		...
		for (y = 0; y <= segY; y++) {
			...
			for (x = 0; x <= segX; x++) {
				...
				var vertex = {
					x: -radius * Math.cos(phiStart + u * phiLength) * Math.sin(thetaStart +
						v * thetaLength),
					y: -radius * Math.cos(thetaStart + v * thetaLength),
					z: radius * Math.sin(phiStart + u * phiLength) * Math.sin(thetaStart +
						v * thetaLength),
		...
		for (y = 0; y < segY; ++y) {
			for (x = 0; x < segX; ++x) {
				...
				dom.perspectiveTransform = new PerspectiveTransform(dom, segWidth,
					segHeight);
				dom.topLeft = vertices[y][x];
				dom.topRight = vertices[y][x + 1];
				dom.bottomLeft = vertices[y + 1][x];
				dom.bottomRight = vertices[y + 1][x + 1];
				domStyle.backgroundPosition = (-segWidth * x) + 'px ' + (-segHeight * y) +
					'px';
	...
	function render() {
		...
		worldBg.style[transformStyleName] = 'scale3d(' + ratio + ',' + ratio +
			',1)';
